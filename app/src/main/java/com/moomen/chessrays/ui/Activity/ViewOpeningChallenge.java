...
public class ViewOpeningChallenge extends AppCompatActivity {
    ...
    private void calculateFrequency() {
        firebaseFirestore.collection("OpeningMoves").whereEqualTo("userId", userUid).get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
            @Override
            public void onComplete(@NonNull Task<QuerySnapshot> task) {
                if (task.isSuccessful()) {
                    if (!task.getResult().isEmpty()) {
                    ...
                    public boolean onMenuItemClick(MenuItem item) {
                        switch (item.getItemId()) {
                            case R.id.popup_view_item_id:
                                OpeningModel openingModel = new OpeningModel(openingMoveUid, openingMoveName);
    ...
    public void onBackPressed() {
        if (!MovesIdStack.moveIdModlesStack.empty()) {
            ...
            if (!MovesIdStack.moveIdModlesStack.empty()) {
                openingModel = MovesIdStack.popStack();
                openingMoveUid = openingModel.getOpeningId();
                openingMoveName = openingModel.getOpeningName();
            }
        }
        super.onBackPressed();
    }
